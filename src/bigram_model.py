...
def bigrams_count_to_probabilities(
    ...
    return (bigram_counts + smooth_factor) / (
        (bigram_counts + smooth_factor).sum(dim=1, keepdim=True)
    )
...
def calculate_neg_mean_log_likelihood(
    ...
    mean_log_likelihood: float = -1.0 * total_log_likelihood / len(words)
...
def generate_name(
    ...
    for _ in range(max_length):
        ...
        current_char = sample_next_character(
            current_char_idx, bigram_probabilities, idx_to_char
        )
...
def calculate_log_likelihood(
    ...
    for bigram in zip(processed_word, processed_word[1:]):
        i: int = char_to_index.get(bigram[0], -1)
        j: int = char_to_index.get(bigram[1], -1)
        if i == -1 or j == -1:
            continue

        log_likelihood += torch.log(bigram_probabilities[i, j])
